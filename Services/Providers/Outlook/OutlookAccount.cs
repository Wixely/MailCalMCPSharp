...
/// <summary>
/// Outlook account backed by Microsoft Graph. v1-skeleton: wiring and capabilities are in
/// place; the data methods are stubbed and will be implemented against <c>GraphServiceClient</c>
/// (built lazily from <see cref="IAuthenticator.AcquireAccessTokenAsync"/>).
/// </summary>
public sealed class OutlookAccount : IMailCalAccount, IMailProvider, ICalendarProvider, IContactsProvider
{
    ...
    {
        MailSearch = true,
        ScheduledSend = true,      // Graph deferred delivery (v2 tool)
        MailRules = true,          // Graph messageRules (v2 tool)
        CalendarTextSearch = true,
        Contacts = true,           // Graph contacts (v2 tool)
    ...
    private static T NotYet<T>(string op) =>
        throw new NotImplementedException($"Outlook.{op} is not implemented in the v1 skeleton yet.");
